...
const updateGame = (p1,p2) => {
  ...
  p1NameDiv.innerText = `${p1}`;
  p2NameDiv.innerText = `${p2}`;
...
class Game {
  ...
  declareWinner(isOver,p1, p2) {
    ...
    let result;
    if(isOver == true && p1 == 0 && p2 > 0) result = resultDiv.innerText = `the winner is p2 with a score of ${p2}`
    
    // Else if isOver is true AND p2 health is <= 0 then update message variable  to 'p2 WINS!'
    else if(isOver == true && p1 > 0 && p2 == 0) result = resultDiv.innerText = `the winner is p1 with a score of ${p1}`;
    // Play victory sound
    const victory_sound = document.getElementById('victory');
...
updateGame(player1.name, player2.name);

// ** Save intial isOver from the game object inside this variable **
let gameState = isOver = false;
